...
router.get('/', async function (req, res) {
    try {
        ...
        let user: ForumUser = <ForumUser>await FM.userManager.getUserBySessionKey(req.cookies["login_id"]);
        ...
        const posts = await FM.forumMediator.postList(user);
...
router.post('/post', async (req, res) => {
    try {
        const user: ForumUser = <ForumUser>await FM.userManager.getUserBySessionKey(req.cookies["login_id"]);
        if (!user) {
